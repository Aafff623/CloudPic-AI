...
本项目采用 **Claude Code** (claude.ai/code) 进行 AI 辅助开发。Claude Code 作为你的 AI 编程搭档，能够帮助你：
...
- **Bug 修复**：快速诊断并解决问题
- **重构优化**：改进代码结构同时保持功能
- **测试驱动**：使用 TDD 方式编写和运行单元/集成测试
- **架构规划**：设计系统改进和新功能

### 使用的 AI 模式

项目利用存储在 `.claude/skills/` 中的多个架构模式：

| 模式 | 描述 |
|------|------|
| `pattern-high-concurrency-realtime-edit` | WebSocket + Disruptor 实现实时协同编辑 |
| `pattern-dynamic-table-sharding` | ShardingSphere 运行时建表实现多租户 |
| `pattern-multi-level-authorization` | Sa-Token + 空间上下文实现多级权限 |
| `pattern-object-storage-pipeline` | COS PicOperations 实现上传时图片处理 |

### 开发工作流

1. **规划**：使用 `/plan` 命令设计功能并评估风险
2. **TDD**：使用 `/tdd` 命令先写测试再实现（强制 80%+ 覆盖率）
3. **代码审查**：使用 `/code-review` 命令验证质量和安全性
4. **构建验证**：使用 `/verify` 命令运行构建和静态分析
...
| `/build-fix` | 修复构建错误或类型问题 |
| `/e2e` | 使用 Playwright 生成 E2E 测试 |

### 项目特色

相比其他教学项目，本项目的 AI 协作特性：

- **架构模式沉淀**：将 WebSocket 高并发、动态分表、多级权限等核心模式沉淀为可复用的技能
- **Test-Driven AI**：强制 AI 先写测试再编码，保证代码质量
- **安全审查**：每次代码变更都经过安全审查，培养安全编码意识
